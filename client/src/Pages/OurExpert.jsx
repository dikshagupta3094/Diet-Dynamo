...
function OurExpert() {
  ...
  const { dietExpert, isLoading, error, _id } = useSelector(
    (state) => state.expert
  );

  const { isSubscribed, loading } = useSelector((state) => state.payment);
          ...
          {dietExpert?.length > 0 ? (
            dietExpert.map((expert) => (
              ...
              >
                ...
                >
                  <NavLink key={expert._id} to={`/postQuery/${expert._id}`}>
                    Post Query
                  </NavLink>
                </button>
